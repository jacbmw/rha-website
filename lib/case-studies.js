...
export async function listPublicCaseStudies() {
  ...
  try {
    // Bounded so a hung dashboard (e.g. upstream 504s) can't stall sitemap
    // generation past Next's 60s static-export limit and fail the build.
    const response = await fetch(source, { headers: { Accept: 'application/json' }, signal: AbortSignal.timeout(15000), next: { revalidate: 900, tags: ['case-studies'] } });
