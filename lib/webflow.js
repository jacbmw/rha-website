...
export async function listBlogItems({ includeDrafts = false, limit = 100, offset = 0 } = {}) {
  ...
  const { categories, authors } = await getLookups();
...
export async function getBlogItem(id) {
  const [item, lookups] = await Promise.all([
    webflowFetch(`/collections/${BLOG_COLLECTION_ID}/items/${encodeURIComponent(id)}`),
    getLookups(),
  ]);
...
export async function createBlogItem(input) {
  ...
  return getBlogItem(item.id);
}

export async function updateBlogItem(id, input) {
  const existing = await webflowFetch(`/collections/${BLOG_COLLECTION_ID}/items/${encodeURIComponent(id)}`);
