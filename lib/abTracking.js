...
// Page-level engagement click (form field focused, Calendly slot picked, …).
// Fires at most once per page load so CTR stays a unique-visitor metric.
const clickedPageKeys = new Set();
export function trackVariantClick() {
  const active = typeof window !== 'undefined' ? window.__rhaPageVariant : null;
  if (!active?.pageKey || clickedPageKeys.has(active.pageKey)) return;
  clickedPageKeys.add(active.pageKey);
  trackVariantEvent('click');
}
