...
function normaliseItem(item) {
  ...
  return {
    ...
    webinarPitch: item.webinarPitch && typeof item.webinarPitch === 'object'
      ? {
        before: String(item.webinarPitch.before || ''),
        linkText: String(item.webinarPitch.linkText || ''),
        after: String(item.webinarPitch.after || ''),
        href: String(item.webinarPitch.href || '/webinar'),
      }
      : null,
    faq: item.faq && Array.isArray(item.faq.items)
      ? {
        items: item.faq.items
          .filter((faqItem) => faqItem && faqItem.question && faqItem.answer)
          .map((faqItem) => ({
...
export async function listBlogItems({ includeDrafts = false, limit = 100, offset = 0, category = '' } = {}) {
