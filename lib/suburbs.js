...
function getWarehousePool() {
  if (!pool) {
    const required = ['RIPEHOUSE_DB_HOST', 'RIPEHOUSE_DB_USER', 'RIPEHOUSE_DB_PASSWORD'];
    const missing = required.filter((key) => !process.env[key]);
    if (missing.length) {
      throw new Error(`Missing warehouse database credentials: ${missing.join(', ')}`);
    }
...
async function loadRegistry() {
  ...
  console.log('[suburbs] registry loaded:', entries.length, 'suburbs');
