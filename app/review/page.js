...
import StickyBookBar from '../discovery-call/StickyBookBar';
...
export default async function ReviewPage({ searchParams }) {
  ...
  const { variant, preview } = await resolveVariant('review', params);
  ...
  return (
    <main className="book-page">
      ...
      <SectionRenderer pageKey="review" sections={content.sections} />

      <Footer />

      <StickyBookBar />
      <VariantTracker pageKey="review" variantId={variant.id} preview={preview} />
    </main>
  );
}
